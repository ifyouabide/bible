...
function toBook(code, contents) {
	...
	let lastKey = Object.keys(out).slice(-1)[0];
	out[lastKey] = out[lastKey].trimRight();
	return [code, out];
}
...
let textContents = fs.readFileSync(path.join('third_party', 'lsv', 'lsv.txt'), 'utf8');
let lsv = toBible(textContents);
writeBibleFilesSync('lsv', lsv);
